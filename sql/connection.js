...
if (
  typeof process.env.MARIADB_USER === "string" &&
  process.env.MARIADB_USER.length
) {
  dbConnectionInfo.user = isFileBased(process.env.MARIADB_USER)
    ? getEnvValueFromFile(process.env.MARIADB_USER)
    : process.env.MARIADB_USER;
} else if (!process.env.MARIADB_SOCKET_PATH) {
  throw new Error(
    "Database user username must be provided with the MARIADB_USER env variable!",
  );
}

if (
  typeof process.env.MARIADB_PASSWORD === "string" &&
  process.env.MARIADB_PASSWORD.length
) {
  dbConnectionInfo.password = isFileBased(process.env.MARIADB_PASSWORD)
    ? getEnvValueFromFile(process.env.MARIADB_PASSWORD)
...
if (
  typeof process.env.MARIADB_HOST_PORT === "string" &&
  process.env.MARIADB_HOST_PORT.length
) {
  const [host, port] = (
    ...
    .split(":")
    .map((item, index) => (index === 1 ? Number.parseInt(item) : item));
...
if (
  typeof process.env.MARIADB_DATABASE_NAME === "string" &&
  process.env.MARIADB_DATABASE_NAME.length
) {
...
if (
  typeof process.env.MARIADB_CONNECTION_LIMIT === "string" &&
  process.env.MARIADB_CONNECTION_LIMIT.length
) {
...
module.exports = {
  getConnection: async function (func) {
    ...
    try {
      ...
      throw new SqlError(
        `Cannot get a connection to mariadb: ${error.message}`,
      );
    }
