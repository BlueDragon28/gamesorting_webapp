...
async function getType(customColumns) {
  ...
  return await mariadb.getConnection(async function (connection) {
    for (let customColumn of customColumns) {
      if (
        !customColumn &&
        (!customColumn.ListColumnTypeID || !customColumn.CustomRowItemsID) &&
        !customColumn.Value
      ) {
        throw new ValueError(400, "Invalid Custom Data Value");
      }
      ...
      if (customColumn.ListColumnTypeID) {
        columnType = await ListColumnType.findByID(
          customColumn.ListColumnTypeID,
          connection
        );
      } else if (
        customColumn.CustomRowItemsID &&
        customColumn.CustomRowItemsID >= 0
      ) {
        columnType = await ListColumnType.findFromUserData(
          customColumn.CustomRowItemsID,
          connection
        );
...
function makeCustomDataNumberValidation() {
  return Joi.object({
    Value: Joi.alternatives().try(
      Joi.number()
        .min(Joi.ref("columnType.min"))
        .max(Joi.ref("columnType.max")),
      Joi.string().trim().min(0).max(0)
    ),
    columnType: Joi.object({
