...
const textValidation = Joi.string()
  ...
  .max(300)
  .required();
const uriValidation = Joi.alternatives()
  .try(
    Joi.string()
      .sanitize()
      .trim()
...
function validateItemHeader(name, url, rating, errorMessages) {
  ...
  return [validatedName, validatedUrl, validatedStar];
}
...
function validateCustomColumns(customColumns, errorMessages) {
  ...
  for (let customColumn of customColumns) {
    const [error, validatedCustomColumn] = validateCustomColumn(customColumn);
...
async function checkIfUserCanCreateAnItem(userID, connection) {
  const isUserBypassingRestriction = await User.isBypassingRestriction(
    userID,
    connection
  );
...
async function updateItem(
  ...
  for (const columnType of listColumnsType) {
    const foundCustomRow = item.customData.find(
      (row) => row.columnTypeID === columnType.id
    );
    if (foundCustomRow) {
  ...
  for (const customColumn of customColumns) {
    ...
    if (customColumn.CustomRowItemsID > 0) {
      foundCustomData = customData.find(
        (data) => data.id == customColumn.CustomRowItemsID
      );
    } else {
      foundCustomData = new CustomRowsItems(
        undefined,
        item.id,
        -customColumn.CustomRowItemsID
      );
