...
function parseCurrentPageHeader(req, res, next) {
  ...
  try {
    ...
    if (
      typeof req.currentPageNumber !== "number" ||
      isNaN(req.currentPageNumber)
    ) {
  ...
  try {
    ...
    if (
      typeof req.currentItemsPageNumber !== "number" ||
      isNaN(req.currentItemsPageNumber)
    ) {
...
router.get(
  ...
  wrapAsync(async function (req, res) {
    ...
    const [lists, pagination] = await existingOrNewConnection(
      null,
...
router.get(
  ...
  wrapAsync(async function (req, res) {
    ...
    const [errorMessage, list] = await existingOrNewConnection(
      null,
      async function (connection) {
...
router.get(
  ...
  wrapAsync(async function (req, res) {
    ...
    if (!req.htmx.isHTMX || (req.htmx.isHTMX && req.htmx.isBoosted)) {
      return res.render(
        "partials/htmx/collections/custom_columns/custom_columns_details.ejs",
        {
          loadingPage: true,
          originalUrl,
        },
    ...
    const [errorMessage, listColumnsType, lists, pagination] =
      await existingOrNewConnection(null, async function (connection) {
        const selectedList = await List.findByID(listID, connection);
...
router.get(
  "/lists/:listID/custom-columns/delete-modal",
  wrapAsync(async function (req, res) {
    const userID = req.session.user.id.toString();
    const { listID } = req.params;
...
router.get(
  "/lists/:listID/custom-columns/edit-form",
  wrapAsync(async function (req, res) {
    const userID = req.session.user.id.toString();
    const { listID } = req.params;
    const { listColumnsTypeID } = req.query;
    ...
    if (!errorMessage) {
      res.render(
        "partials/htmx/collections/custom_columns/partials/update_custom_column_form.ejs",
        {
          listColumnType,
          selectedID: listID,
...
router.get("/lists/:listID/custom-columns/new-form", function (req, res) {
  ...
  res.render(
    "partials/htmx/collections/custom_columns/partials/custom_column_edit_form.ejs",
    {
...
router.get(
  "/lists/:listID/custom-columns/import-from-modal",
  wrapAsync(async function (req, res) {
    const userID = req.session.user.id.toString();
    const { listID } = req.params;
    ...
    const [error, list, searchedLists] = await existingOrNewConnection(
      null,
      async function (connection) {
        const foundList = await List.findByID(listID, connection);
    ...
    return res.render(
      "partials/htmx/modals/importCustomColumnsFromInnerModalList.ejs",
      {
        listID,
        searchedLists,
...
router.delete(
  ...
  wrapAsync(async function (req, res) {
    ...
    const [errorMessage, list, collection] = await existingOrNewConnection(
      null,
      async function (connection) {
        const foundList = await List.findByID(listID, connection);
...
router.get(
  ...
  wrapAsync(async function (req, res) {
    ...
    if (req.htmx.isHTMX && fullPageLoad === "true") {
      const [lists, item, listColumnsType, pagination] =
        await existingOrNewConnection(null, async function (connection) {
          const [lists, pagination] = await List.findFromUser(
            userID,
            connection,
      ...
      const [item, listColumnsType] = await existingOrNewConnection(
        null,
        async function (connection) {
          const currentItem = await Item.findByID(itemID, connection);
...
router.get(
  "/lists/:listID/item/:itemID/delete-modal",
  wrapAsync(async function (req, res) {
    const userID = req.session.user.id;
    const { listID, itemID } = req.params;
    const { destinationId } = req.query;

    const [errorMessage, item] = await existingOrNewConnection(
      null,
      async function (connection) {
...
router.get(
  "/lists/:listID/item/:itemID/edit",
  wrapAsync(async function (req, res) {
    const userID = req.session.user.id.toString();
    const { listID, itemID } = req.params;

    const [errorMessage, list, listColumnsType, item] =
      await existingOrNewConnection(null, async function (connection) {
        const foundList = await List.findByID(listID, connection);
        ...
        return [null, foundList, listColumnsType, foundItem];
      });
...
router.get(
  "/lists/:listID/item/:itemID/move-to",
  wrapAsync(async function (req, res) {
    const userID = req.session.user.id.toString();
    const { listID, itemID } = req.params;
    const isSearchingState = req.get("GS-SearchState") === "true";
    ...
    const [error, searchedLists] = await existingOrNewConnection(
      null,
      async function (connection) {
        const foundItem = await Item.findByID(itemID, connection);
...
router.post(
  ...
  wrapAsync(async function (req, res) {
    ...
    let [collectionName, listName, errorMessage] =
      validateCollectionListName(collectionListName);

    if (!errorMessage) {
      let [validationErrorMessage, collection, list] =
...
router.post(
  ...
  wrapAsync(async function (req, res) {
    ...
    const [validatedName, validatedUrl, validatedRating] = validateItemHeader(
      name,
      url,
      rating,
      errorMessages,
...
router.post(
  "/lists/:listID/switch-list-order",
  wrapAsync(async function (req, res) {
    const userID = req.session.user.id.toString();
    const { listID } = req.params;
...
router.post(
  "/lists/:listID/update-list-sorting",
  wrapAsync(async function (req, res) {
    const userID = req.session.user.id.toString();
    const { listID } = req.params;
...
router.post(
  ...
  wrapAsync(async function (req, res) {
    ...
    const [error, validatedName, validatedType, validatedMin, validatedMax] =
      validateCustomColumn(
        {
          name,
          type,
    ...
    res.render(
      "partials/htmx/collections/custom_columns/partials/details.ejs",
      {
        isValidation: true,
        selectedID: listID,
        existingValues: {
...
router.post(
  "/lists/:listID/custom-columns/import-from",
  wrapAsync(async function (req, res) {
    const userID = req.session.user.id.toString();
    const { listID } = req.params;
    ...
    const [error] = await existingOrNewConnection(
      null,
      async function (connection) {
        try {
        ...
        try {
          await importFromList(listID, importFrom, BigInt(userID), connection);
        } catch (error) {
          return [error.message];
        }
    ...
    if (error) {
      ...
      res.set({
        "HX-Trigger":
          "new-flash-event, close-import-from-modal, update-list-columns-type-list",
      });
    } else {
      res.set({
    ...
    const [errorMessage, listColumnType] = await existingOrNewConnection(
      ...
      async function (connection) {
        const [errorMessage, foundList, listColumnType] =
          await checkIfListColumnTypeOwned(
            userID,
            listID,
            listColumnTypeID,
            connection,
...
router.post(
  ...
  wrapAsync(async function (req, res) {
    ...
    const [error, destinationList, newItem] = await existingOrNewConnection(
      null,
      async function (connection) {
        try {
