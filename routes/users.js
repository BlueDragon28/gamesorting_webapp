...
const {
  isLoggedIn,
  isUserPasswordValid,
} = require("../utils/users/authentification");
...
router.post(
  ...
  wrapAsync(async function (req, res) {
    ...
    const newUser = await existingOrNewConnection(
      null,
      async function (connection) {
        ...
        if (
          await User.checkIfItsDuplicate(
            validatedUsername,
            validatedEmail,
            connection
          )
...
router.post(
  ...
  wrapAsync(async function (req, res) {
    ...
    const { username, password, emptySet } = req.body;

    const errorMessages = {};

    const [validatedUsername, validatedPassword] = validateUserLogin(
      emptySet,
      username,
      password,
      errorMessages
    ...
    const loginUser = await existingOrNewConnection(
      null,
      async function (connection) {
        if (Object.keys(errorMessages).length) {
          return;
...
router.post(
  ...
  wrapAsync(async function (req, res) {
    ...
    const [validatedUsername, validatedPassword] = validateUpdateUsername(
      username,
      password,
      errorMessages
    );
...
router.post(
  ...
  wrapAsync(async function (req, res) {
    ...
    const [validatedEmail, validatedPassword] = validateUpdateEmail(
      email,
      password,
      errorMessages
    );
...
router.delete(
  ...
  wrapAsync(async function (req, res) {
    ...
    const [errorMessage, foundUser] = await existingOrNewConnection(
      null,
      async function (connection) {
        const foundUser = await User.findByID(userID, connection);
...
router.delete(
  ...
  wrapAsync(async function (req, res, next) {
    ...
    const [success, error] = await existingOrNewConnection(
      null,
      async function (connection) {
        try {
          ...
          return [
            false,
            { statusCode: 500, message: `Failed to delete user: ${e.message}` },
          ];
        }
    ...
    const [success, error] = await existingOrNewConnection(
      ...
      async function (connection) {
        if (!currentPassword.length) {
          return [
            false,
            { statusCode: 400, message: "Invalid Current Password" },
          ];
        }
        ...
        if (newPassword !== retypedPassword) {
          return [
            false,
            { statusCode: 400, message: "The Passwords Are Not The Same" },
          ];
        }
