...
const {
  ...
  ValueError,
  InternalError,
} = require("../utils/errors/exceptions");
...
class ListColumnType {
  ...
  async #_exists(connection) {
    ...
    const queryStatement =
      "SELECT COUNT(1) AS count FROM listColumnsType WHERE ListColumnTypeID = ?";
    ...
    try {
      ...
      throw new SqlError(
        `Failed to check if column type item exists: ${error.message}`,
      );
    }
  }
  ...
  async #_isDuplicate(connection) {
    ...
    const queryArgs = [this.name, this.parentList.id ? this.parentList.id : -1];
    ...
    try {
      ...
      throw new SqlError(
        `Failed to check for listColumnType duplicate: ${error.message}`,
      );
    }
  }
  ...
  async #createListColumnType(connection) {
    return await existingOrNewConnection(
      connection,
      this.#_createListColumnType.bind(this),
    );
  }
  ...
  async #_createListColumnType(connection) {
    ...
    try {
      ...
      throw new SqlError(
        `Failed to insert a listColumnsType: ${error.message}`,
      );
    }
  }
  ...
  async #updateListColumnType(connection) {
    return await existingOrNewConnection(
      connection,
      this.#_updateListColumnType.bind(this),
    );
  }
    ...
    return await existingOrNewConnection(
      ...
      async function (connection) {
        ...
        try {
          ...
          const foundColumnType = new ListColumnType(
            queryResult[0].Name,
            queryResult[0].Type,
            foundList,
          );
          ...
          throw new SqlError(
            `Failed to get column type from ID: ${error.message}`,
          );
        }
      },
    );
    ...
    return await existingOrNewConnection(
      ...
      async function (connection) {
        ...
        try {
          ...
          throw new SqlError(
            `Failed to get column type by name: ${error.message}`,
          );
        }
      },
    );
    ...
    return await existingOrNewConnection(
      ...
      async function (connection) {
        ...
        try {
          ...
          throw new SqlError(
            `Failed to get column type from user data id: ${error.message}`,
          );
        }
      },
    );
  ...
  static async deleteFromList(listID, connection) {
    ...
    return await existingOrNewConnection(
      connection,
      async function (connection) {
        const queryStatement = `DELETE FROM listColumnsType WHERE ListID = ?`;

        const queryArgs = [listID];

        try {
          const queryResult = await connection.query(queryStatement, queryArgs);
        } catch (error) {
          throw new SqlError(
            `Failed to delete columns type from list id: ${error.message}`,
          );
        }
      },
    );
  ...
  static async deleteFromID(customListID, connection) {
    ...
    return await existingOrNewConnection(
      connection,
      async function (connection) {
        const queryStatement = `DELETE FROM listColumnsType WHERE ListColumnTypeID = ${customListID}`;

        const queryArgs = [customListID];
        ...
        try {
          ...
          throw new SqlError(
            `Failed to delete custom column ${customListID}: ${error.message}`,
          );
        }
      },
    );
  ...
  static async getCount(listID, connection) {
    ...
    return await existingOrNewConnection(
      connection,
      async function (connection) {
        const queryStatement = `SELECT COUNT(*) AS count FROM listColumnsType WHERE ListID = ${listID}`;
        ...
        try {
          ...
          throw new SqlError(
            `Failed to query number of customs columns: ${error.message}`,
          );
        }
      },
    );
    ...
    return await existingOrNewConnection(
      ...
      async function (connection) {
        ...
        try {
          ...
          throw new SqlError(
            `Failed to query the number of custom columns from user: ${error.message}`,
          );
        }
      },
    );
