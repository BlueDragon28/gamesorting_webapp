...
function addSearchOptions(searchOptions, queryArgs) {
  if (
    !searchOptions ||
    typeof searchOptions.exactMatch !== "boolean" ||
    typeof searchOptions.regex !== "boolean" ||
    typeof searchOptions.text !== "string" ||
    !searchOptions.text.length
  ) {
    return "";
  }
...
class Item {
  ...
  toBaseObject() {
    ...
    return {
      ...
      customData: this.customData
        ? this.customData.map((item) => item.toBaseObject())
        : null,
  ...
  async #_exists(connection) {
    ...
    const queryStatement =
      "SELECT COUNT(1) AS count FROM items WHERE ItemID = ?";
  ...
  async #_createItem(connection) {
    const queryStatement =
      `INSERT INTO items(ListID, Name ${this.url.length > 0 ? ", URL" : ""} ${
    ...
    const queryArgs = [this.parentList.id, this.name];
  ...
  async #_updateItem(connection) {
    let queryStatement = "UPDATE items SET Name = ?, Rating = ?";

    const queryArgs = [this.name, this.rating];
  ...
  async #_isDuplicate(connection) {
    ...
    const queryArgs = [this.name, this.id ? this.id : -1, this.parentList.id];
  ...
  static async findFromList(
    ...
    return await existingOrNewConnection(
      connection,
      async function (connection) {
        const foundListSorting =
          listSorting instanceof ListSorting
            ? listSorting
            : await ListSorting.findByList(list, connection);

        // If page number if to high, set it to 1
        const [isValidPageNumber, numberOfItems] = await Item.isValidPageNumber(
          list,
          pageNumber,
          connection,
          searchOptions
  ...
  static async deleteFromID(id, connection) {
    ...
    return await existingOrNewConnection(
      connection,
      async function (connection) {
        const queryStatement = "DELETE FROM items WHERE ItemID = ?";
  ...
  static async isValidPageNumber(
    list,
    pageNumber,
    connection,
    searchOptions = null
